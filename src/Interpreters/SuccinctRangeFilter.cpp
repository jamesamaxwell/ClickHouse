        ...
        else
        {
            ...
            else
            {
                ...
                { // pos - rank1(S-HasChild, pos) - 1.
                    iter.valuePosition = rank1(surf.sparse.s_hasChild, iter.sparsePos) - 1;
                    while (!surf.sparse.s_LOUDS[iter.sparsePos])
                    {
                        if (surf.sparse.s_labels[iter.sparsePos] == 0xFF)
                        {
                            iter.valuePosition = rank1(surf.sparse.s_hasChild, iter.sparsePos) - 1;
                            iter.currentLevel = level;
                            LOG_DEBUG(getLogger("SuccinctRangeFilter"), "valuePos sparse: {}", iter.valuePosition);
                            return iter;
                        }
                        iter.sparsePos++;
                    }
                ...
                // else if (surf.sparse.s_hasChild[iter.sparsePos])
                // {
                //     iter.levelPositions.push_back(iter.sparsePos);
                //     iter
                // }
            }
        }
...
//             bool isPrefixKey = surf.dense.d_isPrefixKey[(posDense / 256) + older_children];
//             if (isPrefixKey && (!exact_match || level == key.size() - 1))
//             {
                
//                 iter.valuePosition = rank1(surf.dense.d_labels, posDense + candidate)
//                                     - rank1(surf.dense.d_hasChild, posDense + candidate)
//                                     + rank1(surf.dense.d_isPrefixKey, posDense / 256)
//                                     - 1;
//                 iter.currentLevel = level;
//                 LOG_DEBUG(getLogger("SuccinctRangeFilter"), "is prefix key return, value position: {}", iter.valuePosition);
//                 return iter;
//             }

//             const std::bitset<256> & node_labels = surf.dense.d_labels[(posDense / 256) + older_children];
//             const std::bitset<256> & has_child = surf.dense.d_hasChild[(posDense / 256) + older_children];
...
//                                             + rank1(surf.dense.d_isPrefixKey, posDense / 256)
//                                             - 1;
//                         LOG_DEBUG(getLogger("SuccinctRangeFilter"), "valuePos dense: {}", iter.valuePosition);
//                     }
//                     if (!has_child.test(candidate) && !exact_match)
//                     {
//                         foundCandidate = true;
...
//             if (!surf.dense.d_hasChild[block_index].test(bit_offset + candidate))
//             {
...
//             // uint8_t s_node_labels;
//             // if (level == l_depth)
//             // {
//             //     s_node_labels = surf.sparse.s_labels[(posDense / 256) + older_children - surf.dense.d_labels.size()];
//             // }
//             // else
//             // {
//             //     s_node_labels = surf.sparse.s_labels[posSparse];
//             // }
//         }
//     }

//     posSparse = 0; // start at beginning of sparse part
